...
const paralyzer: Theme = {
  ...
  colors: {
    ...
    'terminal.ansiBrightBlack': colors.whiteDim,
  ...
  tokenColors: [
    ...
    },
    {
      scope: [1, 3, 5, 7].map((depth) => {
        const metas = (new Array(depth)).fill('').map(() => 'meta.structure.dictionary.json').join(' ');
